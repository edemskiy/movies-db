...
export function MoviePage({ match }) {
  ...
  function getMainCrew(crew) {
    return [
      crew.filter((member) => member.job === "Director")[0],
      crew.filter((member) => member.job.match(/writer|screenplay|consultant/i))[0],
      crew.filter((member) => member.job.match(/composer|^music$/i))[0],
    ].filter(Boolean);
  }
  useEffect(() => {
    request(getMovieDetailsURL(movie_type, movie_id)).then((movie) => setMovie({ ...movie, media_type: movie_type }));
